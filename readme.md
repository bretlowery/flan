...
| -c | Continuous streaming mode. If specified, enables continuous streaming. Use with -o to direct output to a target source. Use -s to specify a start timestamp. -e, -r, -n will be ignored. Will stream output as fast as possible unless --pace is specified. | No continuous streaming |
