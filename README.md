...
| &#x2011;o | Stream mode output target. If specified, ignores the output directory and -n flag values, enables quiet mode (-q), and streams all output to the designated target. One of:<br><br>kafka=stream to a Kafka pub/sub as defined in the flan.kafka.yaml config file<br><br>splunk=stream to a Splunk Enterprise API as defined in the flan.splunk.config file<br><br>stompmq=stream to a Apache ActiveMQ, Amazon MQ, RabbitMQ, or other STOMP-compliant queue as defined in the flan.stompmq.yaml config file<br><br>stdout=stream to stdout | No stream output. Output is written to file(s) in the output directory provided. |
