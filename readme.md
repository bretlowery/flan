...
| -m,<br>--ipmapping | Defines how IPs are obfuscated, one of: onetomany=one template log IP is mapped to one or more obfuscated IPs in the generated logs; this is the most random obfuscation but destroys sessions. onetoone=maps every template log IP to a single obfuscated IP in the generated logs, preserving sessions; none=no IP obfuscation, IPs are left as-is. | onetomany |
